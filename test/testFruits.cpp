...
TEST_CASE("Fruit Visibility", "[fruits]") {
  ...
  SECTION("9 seconds but no pellets eaten") {
    ms_pacman::updateFruit(gameState.currentFruit, std::chrono::milliseconds(9001), gameState.score.eatenPellets);
    REQUIRE_FALSE(getFruitVisibility(gameState.currentFruit));
  }
  ...
  SECTION("9 seconds and 70 pellets eaten") {
    ...
    ms_pacman::updateFruit(gameState.currentFruit, std::chrono::milliseconds(1), gameState.score.eatenPellets);
  ...
  SECTION("Eating a fruit") {
    REQUIRE(eatFruits(gameState.currentFruit) == 0);

    gameState.score.eatenPellets = 70;
    ms_pacman::updateFruit(gameState.currentFruit, std::chrono::milliseconds(1), gameState.score.eatenPellets);
    REQUIRE(getFruitVisibility(gameState.currentFruit));
    REQUIRE(eatFruits(gameState.currentFruit) == ms_pacman::getFruitValue(gameState.currentFruit));
