...
TEST_CASE("Pellet initialization", "[pellets]") {
  ms_pacman::Pellets pellets;
  pelletInitTest(pellets, 240, 1, 9);

  ms_pacman::SuperPellets superPellets;
...
static void pelletEatingTest(T & pellets, size_t countBefore, size_t countAfter, size_t x, size_t y) {
  ...
  if (countBefore != countAfter) {
    REQUIRE(pellets.eatPelletAtPosition(ms_pacman::GridPosition{ x, y }));
  } else {
    REQUIRE_FALSE(pellets.eatPelletAtPosition(ms_pacman::GridPosition{ x, y }));
  }
