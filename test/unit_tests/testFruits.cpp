...
TEST_CASE("Fruit Visibility", "[fruits]") {
  ...
  SECTION("9 seconds but no pellets eaten") {
    ms_pacman::Fruits::update(gameState.currentFruit(), std::chrono::milliseconds(9001), gameState.score.eatenPellets);
    REQUIRE_FALSE(ms_pacman::Fruits::isVisible(gameState.currentFruit()));
  }
  ...
  SECTION("9 seconds and 70 pellets eaten") {
    ...
    ms_pacman::Fruits::update(gameState.currentFruit(), std::chrono::milliseconds(1), gameState.score.eatenPellets);
